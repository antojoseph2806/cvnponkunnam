...
const Header = () => {
  ...
  const desktopLinkClass = (href: string) =>
    `text-sm font-medium transition-colors tracking-wide ${pathname === href ? "text-primary" : "text-foreground hover:text-primary"}`;
    ...
    <header className="w-full fixed top-0 left-0 z-50">
      ...
      <div className="bg-primary text-primary-foreground text-sm py-2 px-4 md:px-8 lg:px-16 flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 ml-2">
            ...
            <button type="button" aria-label="Instagram" className="hover:text-highlight-light transition-colors">
            ...
            <button type="button" aria-label="WhatsApp" className="hover:text-highlight-light transition-colors">
          ...
          {navItems.map((item) =>
            item.children ? (
              <div key={item.label} className="relative group">
                <Link href={item.href} className={`flex items-center gap-1 ${desktopLinkClass(item.href)}`}>
                  {item.label}
                ...
                <div className="absolute top-full left-0 mt-2 bg-card shadow-lg rounded-md py-2 min-w-[200px] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                  {item.children.map((child) => (
                    <Link key={child.label} href={child.href} className="block px-4 py-2 text-sm text-foreground hover:bg-muted hover:text-primary transition-colors">
                      {child.label}
                    </Link>
                  ))}
                </div>
              ...
              <Link key={item.label} href={item.href} className={desktopLinkClass(item.href)}>
                {item.label}
              </Link>
            )
          )}
          <a href="https://wa.me/917306901750" target="_blank" rel="noopener noreferrer" className="ml-4 bg-primary text-primary-foreground px-5 py-2.5 rounded text-sm font-semibold hover:bg-accent transition-colors">
            SEND A MESSAGE
          </a>
        </div>
      ...
      {mobileOpen && (
        <div className="lg:hidden bg-card border-t border-border shadow-lg">
            ...
            item.children ? (
              <div key={item.label}>
                <div className="w-full flex items-center justify-between px-6 py-3">
                  <Link href={item.href} onClick={() => setMobileOpen(false)} className="text-sm font-medium text-foreground hover:text-primary">
                    {item.label}
                  </Link>
                  <button type="button" onClick={() => setServicesOpen(!servicesOpen)} className="text-foreground">
                  ...
                  <div className="bg-muted">
                    {item.children.map((child) => (
                      <Link
                        ...
                        className="block px-10 py-2 text-sm text-foreground hover:text-primary"
                      >
                        {child.label}
                      </Link>
                    ))}
          ...
          <div className="p-4">
            <a href="https://wa.me/917306901750" target="_blank" rel="noopener noreferrer" className="block text-center bg-primary text-primary-foreground px-5 py-2.5 rounded text-sm font-semibold">
              SEND A MESSAGE
            </a>
          </div>
        </div>
